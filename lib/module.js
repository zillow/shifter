...
var buildJS = function (mod, name, callback) {
    ...
    qu = queue.log('writing RAW file')
        .check()
        .write(path.join(mod.buildDir, fileName, fileName + '.js'));

    if (compressorFn !== 'none') {
        qu.log('compressing ' + path.join(fileName, fileName + '.js with ' + cName[compressorFn]));
        qu = queue[compressorFn](compressorConfig)
            .log('writing -min file')
            .check()
            .write(path.join(mod.buildDir, fileName, fileName + '-min.js'));
    }

    qu.run(function (err, result) {
        if (err) {
            if (/file has not changed/.test(err)) {
                log.warn(name + ': ' + err);
...
var buildCoverage = function (mod, name, callback) {
    ...
    queue.read([
        ...
        .coverage({
            type: coverageType,
            charset: 'utf8',
            name: mod.buildDir + '/' + fileName + '/' + fileName + '.js'
        })
        .replace(replaceOptions)
        .check()
